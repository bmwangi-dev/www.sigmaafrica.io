...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-xl text-sm font-bold ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "bg-[var(--color-migenta)] text-white hover:brightness-110 shadow-lg active:scale-[0.98]",
        destructive:
          "bg-destructive text-destructive-foreground hover:bg-destructive/90",
        outline:
          "border-2 border-[var(--color-migenta)] bg-transparent text-[var(--color-migenta)] hover:bg-[var(--color-migenta)] hover:text-white",
        secondary:
  ...
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        ref={ref}
        {...props}
        disabled={props.disabled || props.type === 'submit'}
        aria-disabled={props.disabled || props.type === 'submit'}
      />
    )
  }
)
